...
**A segunda fábrica: o mesmo jogo, igualzinho, começando do zero.**

É a [Fábrica de Emojis](../fabrica-de-emojis/) inteira — os mesmos 800 emojis,
120 raridades, 340 upgrades, 80 conquistas, 10 temporadas, 18 temas, 18 skins,
4 modos e os 12 espaços de jogo salvo. A única diferença é que ela tem a
**caderneta de saves só dela** (`fabricaEmojis2_*` em vez de `fabricaEmojis_*`),
então dá pra recomeçar tudo aqui sem encostar num fio do jogo antigo — e dá pra
ter as duas instaladas ao mesmo tempo no celular, cada uma com o ícone dela
(a 1 é dourada, a 2 é verde e tem um **2** no canto).
...
> Mudou alguma coisa na fábrica 1? Vale trazer pra cá também: as duas são o
> mesmo `index.html`, mudando só o título, a marca, o ícone, o `sw.js` e o
> prefixo das chaves de save.
...
- Direto: `https://willianwiab.github.io/contas-wen/fabrica-de-emojis-2/`
- No portfólio **Jogos do JoJo**: aparece na página inicial, na lista de jogos e
  tem a página própria dele em `jogos-do-jojo/pages/jogo.html?id=fabrica-de-emojis-2`.
  O cadastro fica em `jogos-do-jojo/data/games.json`.
- A fábrica original continua em `../fabrica-de-emojis/`, com o save dela
  intacto.
...
- Cada jogo salvo fica no `localStorage` em `fabricaEmojis2_jogo_<id>`, com a lista em `fabricaEmojis2_jogos` (formato v6). O código de migração de saves antigos continua aqui, mas procura pelas chaves `fabricaEmojis2_*` — que não existem numa fábrica nova. É de propósito: assim a fábrica 2 nunca lê nem escreve nada da fábrica 1.
