...
(async () => {
  ...
  await ctx.addInitScript(() => {
    localStorage.setItem('fabricaEmojis2_v5_normal', JSON.stringify({
  ...
  r = await p.evaluate(() => { dinheiro = 555555; salvarNaMao();
    const g = JSON.parse(localStorage.getItem('fabricaEmojis2_jogo_'+jogoAtual.id));
  ...
  r = await p.evaluate(() => ({ jogos: jogos.length, chaves: Object.keys(localStorage).filter(k=>k.startsWith('fabricaEmojis2_jogo_')).length }));
