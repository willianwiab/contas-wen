/* ============================================================
   FÁBRICA DE EMOJIS 2 — teste de aplicativo (PWA)
...
const base = 'http://127.0.0.1:8822/fabrica-de-emojis-2/';
...
(async () => {
  ...
  conf('manifesto carrega com nome, ícones e display standalone',
    r.nome==='Fábrica de Emojis 2' && r.display==='standalone' && r.icones===4 && r.todosOk && r.apple, JSON.stringify(r));

  // service worker registra e assume a página
  r = await p.evaluate(async () => {
    const reg = await navigator.serviceWorker.ready;
    return { ativo: !!reg.active, escopo: reg.scope.endsWith('/fabrica-de-emojis-2/') };
  });
  ...
  conf('abre sem internet, com o jogo salvo intacto', r.titulo==='FÁBRICA DE EMOJIS 2' && r.jogo===340 && r.jogos===1, JSON.stringify(r));
