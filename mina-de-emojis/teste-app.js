/* ============================================================
   MINA DE EMOJIS — teste de aplicativo (PWA)
...
const base = process.env.BASE || 'http://127.0.0.1:8822/mina-de-emojis/';
...
(async () => {
  ...
  conf('manifesto carrega com nome, ícones e display standalone',
    r.nome==='Mina de Emojis' && r.display==='standalone' && r.icones===4 && r.todosOk && r.apple, JSON.stringify(r));

  // service worker registra e assume a página
  r = await p.evaluate(async () => {
    const reg = await navigator.serviceWorker.ready;
    return { ativo: !!reg.active, escopo: reg.scope.endsWith('/mina-de-emojis/') };
  });
  conf('service worker registrado e ativo', r.ativo && r.escopo, JSON.stringify(r));

  // joga um pouco e salva
  await p.evaluate(() => { for(let i=0;i<12;i++) cavarProxima(); salvar(); });
  ...
  r = await p.evaluate(() => ({ titulo: document.title, jogo: typeof LOJA !== 'undefined' && LOJA.length,
    fundura: typeof fundura !== "undefined" ? fundura : -1,
    temMina: !!document.getElementById('mina') }));
  conf('abre sem internet, com o jogo salvo intacto', r.titulo==='MINA DE EMOJIS' && r.jogo===48 && r.fundura>0, JSON.stringify(r));
