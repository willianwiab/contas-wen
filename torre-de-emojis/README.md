# 🗼 Torre de Emojis

O bloco vai e volta lá em cima; você solta e ele empilha. Quanto mais alta a
torre, mais dinheiro cada bloco vale. Errar demais entorta a torre — e torre
torta cai. Um arquivo só (`index.html`), funciona offline no celular e no PC.

## Como se joga

- Clique na tela (ou aperte **espaço**) pra soltar o bloco.
- Acertar **bem no meio** do bloco de baixo é **PERFEITO**: paga **3x** e ainda
  endireita um pouco a torre.
- Bloco que erra feio **cai fora** e custa uma vida. Sem vidas, acabou.
- Cada bloco fora do lugar **entorta** a torre. Entortou demais, ela desaba.
- O dinheiro **fica guardado** entre uma rodada e outra — é com ele que você
  compra as melhorias.

## As camadas

Conforme a torre sobe, você atravessa o céu inteiro. Cada camada tem o próprio
visual (nuvens, meteoro, aurora, estrelas, buraco negro...), o próprio vento e
**paga mais que a de baixo**:

| | Camada | Altura | Paga |
| --- | --- | --- | --- |
| 🌱 | Terra | 0 | x1 |
| ☁️ | Troposfera | 10 | x1,6 |
| 🎈 | Estratosfera | 25 | x2,6 |
| ☄️ | Mesosfera | 45 | x4,2 |
| 🛰️ | Termosfera | 70 | x7 |
| 🌌 | Exosfera | 100 | x12 |
| 🚀 | Espaço | 140 | x22 |
| 🪐 | Universo | 190 | x45 |
| 🕳️ | Fim do Espaço | 250 | x95 |
| 🌑 | Fim do Mundo | 320 | x200 |
| ⬛ | **o NADA** | 400 | x500 |

## As melhorias (136, oito níveis de cada)

| | O que faz |
| --- | --- |
| 💰 **Contrato** | cada bloco vale mais |
| 🧲 **Cola** | a torre entorta menos |
| 🐢 **Freio** | o bloco anda mais devagar (fica bem mais fácil acertar) |
| ❤️ **Capacete** | mais vidas por rodada |
| 📏 **Bloco** | blocos mais largos |
| 🪜 **Andaime** | já começa a rodada com blocos prontos |
| 🎯 **Mira** | a zona do PERFEITO fica maior |
| 🪝 **Guindaste** | o bloco balança menos longe |
| 💎 **Bloco de Ouro** | chance de sair um bloco que paga 10x |
| ⚖️ **Prumo** | a torre se endireita sozinha a cada bloco |
| 🌬️ **Para-vento** | o vento das camadas altas atrapalha menos |
| 📈 **Sequência** | PERFEITOs seguidos pagam cada vez mais |
| 🍀 **Sorte** | chance do bloco encaixar sozinho no meio |
| 🏦 **Poupança** | rende dinheiro por segundo, mesmo parado |
| 🏅 **Fama** | ganha mais por cada camada que você já alcançou |
| 🚀 **Foguete** | começa a rodada mais alto por camada aberta |
| ⚡ **Núcleo** | multiplica tudo |

As melhorias que "seguram" alguma coisa (Cola, Freio, Mira, Prumo, Para-vento)
usam uma curva que **chega perto do limite mas nunca em 100%** — senão dava pra
comprar tanto que o jogo perdia a graça.

## As cestas

Trocam os emojis da torre. Abrem conforme o seu **recorde de altura**:

| Cesta | Abre em |
| --- | --- |
| 😀 Carinhas | desde o começo |
| 🐯 Bichos | 25 blocos |
| 🍕 Comida | 60 blocos |
| 🚀 Espaço | 120 blocos |
| 🔮 Magia | 200 blocos |
| ⬛ o NADA | 400 blocos |

## Instalar como aplicativo

No Android: Chrome → menu ⋮ → *Instalar aplicativo* (ou o botão **📲 INSTALAR**).
No iPhone: Safari → compartilhar → *Adicionar à Tela de Início*. No computador:
o ⊕ na barra de endereço. Depois de abrir uma vez, funciona sem internet.

## Testando

Com `npm i playwright-core`:

- `node teste-torre.js` — 11 checagens: as camadas na ordem certa, o encaixe com
  a torre torta, a loja das 17 famílias, a poupança, o desenho de todas as
  camadas e o save.
- `node teste-app.js` — 4 checagens de aplicativo (precisa servir por http:
  `python3 -m http.server 8822` na raiz do repositório).

## Detalhes técnicos

- Os blocos não giram: cada um encaixa no topo do anterior, e o que decide tudo
  é a distância entre os dois centros. É o que deixa a pilha estável em vez de
  tremer.
- A torre inteira é **desenhada inclinada** quando entorta, então a mira é
  convertida pro "espaço da torre" antes de decidir o encaixe (`paraTorre()`).
  Sem isso o jogo cobrava uma posição e a tela mostrava outra — numa torre de 30
  blocos a diferença passava de 90 pixels, e ficava impossível acertar.
- O ângulo da inclinação diminui conforme a torre cresce: um anguinho de nada já
  jogaria o topo de uma torre de 200 blocos pra fora da tela.
- O **desvio** da torre é a soma das distâncias de cada bloco pro de baixo — ou
  seja, o quanto o topo saiu de cima da base. Passou de um bloco e pouco, desaba.
- O save (`torreEmojis_v1`) guarda dinheiro, recorde, melhorias, cesta e álbum.
