/* ============================================================
   中國表情符號工廠 — teste dos jogos salvos
...
(async () => {
  ...
  await ctx.addInitScript(() => {
    localStorage.setItem('fabricaChina_v5_normal', JSON.stringify({
  ...
  r = await p.evaluate(() => ({ jogos: jogos.length, chaves: Object.keys(localStorage).filter(k=>k.startsWith('fabricaChina_jogo_')).length }));
