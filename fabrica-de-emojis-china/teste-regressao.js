...
(async () => {
  ...
  let r = await p.evaluate(() => ({ inicio: document.getElementById('inicio').classList.contains('on'),
    ...
    naoSalvou: !Object.keys(localStorage).some(k => k.startsWith('fabricaChina_jogo_')) }));
  ...
  // 6. datas — aqui os festivais seguem a lua, então vêm da tabela LUNARES
  r = await p.evaluate(() => {
    ...
    return { ano:qual(2026,2,18), lanternas:qual(2026,3,3), outono:qual(2026,9,25),
      nacional:qual(2026,10,3), solteiros:qual(2026,11,11), solteiros10:qual(2026,11,10),
      cny26:iso(festival(2026,CNY)), cny27:iso(festival(2027,CNY)) }; });
  conf('calendário chinês certo (春節 2026 = 17/02 e 2027 = 06/02, 雙十一 só no dia 11)',
    r.ano==='chunjie' && r.lanternas==='yuanxiao' && r.outono==='zhongqiu' &&
    r.nacional==='guoqing' && r.solteiros==='shuang11' && r.solteiros10===null &&
    r.cny26==='2026-02-17' && r.cny27==='2027-02-06',
    JSON.stringify(r));
  ...
  r = await p.evaluate(() => { cristais = 300; montarTemas(); const antes = skinAtual;
    ...
    const bloq = (() => { const a = skinAtual; comprarSkin('schunjie'); return skinAtual === a; })();
  ...
  r = await p.evaluate(() => ({ jogos: jogos.length,
    chaves: Object.keys(localStorage).filter(k=>k.startsWith('fabricaChina_jogo_')).length }));
