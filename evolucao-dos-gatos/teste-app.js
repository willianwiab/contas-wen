/* ============================================================
   EVOLUÇÃO DOS GATOS — teste de aplicativo (PWA)
...
const base = 'http://127.0.0.1:8822/evolucao-dos-gatos/';
...
(async () => {
  ...
  conf('manifesto carrega com nome, ícones e display standalone',
    r.nome==='Evolução dos Gatos' && r.display==='standalone' && r.icones===4 && r.todosOk && r.apple, JSON.stringify(r));

  // service worker registra e assume a página
  r = await p.evaluate(async () => {
    const reg = await navigator.serviceWorker.ready;
    return { ativo: !!reg.active, escopo: reg.scope.endsWith('/evolucao-dos-gatos/') };
  });
  conf('service worker registrado e ativo', r.ativo && r.escopo, JSON.stringify(r));

  // caça um pouco e salva
  await p.evaluate(() => { nivel = 12; peixes = 999; ganharXp(50);
    document.getElementById('evo').classList.remove('on'); salvar(); });
  ...
  r = await p.evaluate(() => ({ titulo: document.title,
    brinquedos: typeof LOJA !== 'undefined' && LOJA.length,
    nivel: typeof nivel !== 'undefined' ? nivel : -1,
    peixes: typeof peixes !== 'undefined' ? Math.round(peixes) : -1 }));
  conf('abre sem internet, com o jogo salvo intacto',
    r.titulo==='EVOLUÇÃO DOS GATOS' && r.brinquedos===96 && r.nivel>=12, JSON.stringify(r));
